...
exports.createTenant = async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (prisma) => {
            ...
            if (password && phone) {
                const message = `Your credentials for the Property Management App:\nEmail: ${email}\nPassword: ${password}\nLogin here: ${process.env.FRONTEND_URL || `https://masteko-property-kti.netlify.app` || 'http://localhost:5173'}/login`;
...
exports.sendInvite = async (req, res) => {
    try {
        ...
        res.json({
            ...
            inviteLink: `${process.env.FRONTEND_URL || `https://masteko-property-kti.netlify.app` || 'http://localhost:5173'}/tenant/invite/${user.inviteToken}`
        });
    } catch (error) {
