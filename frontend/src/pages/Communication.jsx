...
const Communication = () => {
  ...
  const getFilteredConversations = () => {
    ...
    if (activeTab !== 'ALL') filtered = filtered.filter(u => u.role === activeTab);
    if (searchTerm) {
        ...
        <div className="w-[340px] border-r border-slate-100 flex flex-col bg-slate-50/50">
          <div className="p-4 bg-white border-b border-slate-100 space-y-4">
            ...
            <div className="flex p-1 bg-slate-100 rounded-lg overflow-x-auto scrollbar-hide">
              {['ALL', 'TENANT', 'OWNER', 'AUDIT'].map(tab => (
                <button
                  key={tab}
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === 'AUDIT' ? (
              ...
              <div
                ...
                className={`p-4 flex items-center gap-3 cursor-pointer border-b border-slate-50 hover:bg-white group ${selectedUser?.id === user.id ? 'bg-white border-l-4 border-l-indigo-600' : 'border-l-4 border-l-transparent'}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold truncate text-slate-700">{user.name || user.email}</h4>
