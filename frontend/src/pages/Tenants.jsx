...
export const Tenants = () => {
  ...
  const handleSaveTenant = async (e) => {
    ...
    try {
      ...
      console.error(e);
      alert(editingTenant ? 'Failed to update tenant' : 'Failed to create tenant');
    }
  };
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-slate-800">{editingTenant ? 'Edit Tenant' : 'Add Tenant'}</h3>
                <button type="button" onClick={() => { setShowModal(false); setEditingTenant(null); resetForm(); }} className="text-slate-400 hover:text-slate-600">
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4 p-4 bg-indigo-50/30 rounded-xl border border-indigo-100/50">
                      <div className="flex justify-between items-center">
                        <h5 className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Residents / Occupants</h5>
                        <button
                          type="button"
                          onClick={() => setResidents([...residents, { firstName: '', lastName: '' }])}
                          className="text-xs font-bold text-indigo-600 hover:text-indigo-700 underline"
                        >
                          + Add Resident
                        </button>
                      ...
                      {residents.length === 0 && (
                        <p className="text-xs text-slate-400 italic">No additional residents added.</p>
                      )}
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-sm font-medium text-slate-600">Building Name</label>
                      <div className="relative">
                        <select
                          name="propertyId"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                          ...
                          >
                            ...
                            {availableUnits.map(u => (
                              <option key={u.id} value={u.id}>
                                {u.unit_identifier || u.unitIdentifier || u.unitNumber || u.name}
                              </option>
                            ))}
                          </select>
                          <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">▼</div>
                      ...
                      {isBedroomWise && (
                        <div className="flex flex-col gap-2 animate-in fade-in zoom-in-95 duration-200">
                          <label className="text-sm font-medium text-slate-600">Bedroom</label>
                          <div className="relative">
                            <select
                              name="bedroomId"
                ...
                {/* Invite Information (Note instead of Password) */}
                {!editingTenant && (
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                <Button type="submit" variant="primary" disabled={loading}>
                  {editingTenant ? 'Update Tenant' : 'Save & Send Invite'}
                </Button>
              </div>
            </form>
          </div>
